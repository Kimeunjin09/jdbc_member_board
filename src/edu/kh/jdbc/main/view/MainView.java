...
import edu.kh.jdbc.main.model.sercive.MainService;
...
public class MainView {
	...
	// 이걸 회원만 볼수있는 메뉴에 전달을 해줘야 하니까 매개변수로 전달해줘야함(MemberView한테)
		...
		do {
			...
			try {
				...
				if(LoginMember == null) { // 로그인 X
					...
					// 로그인된 회원의 정보를 전달해줘야 memberMenu가 돌아가니까, 전달 매개변수(LoginMember)존재
					...
					case 0: 
						...
						//break; // break를 밑에 99가 실행되면서 프로그램 종료되버림
					case 99: System.out.println("프로그램 종료");
							 System.exit(0);
					         // System.exit(0); --> JVM 종료, 매개변수는 0 ( 0 아니면 오류를 의미 )
					         // 프로그램 종료시 JVM까지 멈추고 싶을 때 사용
					         // 컴파일러가 자동삽입해주기 때문에 명시적 작성임, 보통 Run에서 작성
							   break;
					default : System.out.println("메뉴에 작성된 번호만 입력해주세요.");
					}
				}
	...
	private void login() {
		...
		try {
			...
			// 입력받은 아이디, 비번을 service에있는 로그인 메서드에 전달하기 위해 매개변수로 전달
	...
	private void signUp() {
		...
		try {
			...
			while(true) {
				...
				// 중복 여부를 검사하는 서비스 호출 후 결과 반환 받기(있으면 1 / 없으면 0)
				...
				if(result == 0) { // 중복이 아닌경우
					...
					break; // true이면 while문이 무한반복이니까 break(가장 가까운 반복문 탈출)
			...
			while(true) {
				...
				System.out.println();
				// 이름 아이디 비밀번호 성별 외에는 디폴트값이 존재하니까 입력받지 않음
			...
			int result = service.singUp(member); // 회원가입은 insert니까 행의 갯수 반환 int형
